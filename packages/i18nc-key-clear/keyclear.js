...
const debug = require('debug')('i18nc-key-clear');
	...
	i18nc.registerPlugin('keyClear', function(i18nc, settings, enabled) {
		...
		i18nc.addListener('cutword', function(emitData) {
			if (emitData.options.pluginEnabled.keyClear) {
				...
				emitData.result = keyClear(
					emitData.result,
					emitData.originalString
				);
			} else {
...
function keyClear(lineStrings, value) {
	const commentIndexs = _commentIndexs(value);
...
function _commentIndexs(value) {
	const commentAllIndexItems = [];
	const commentStartReg = /<!--/g;
	const commentEndReg = /-->/g;
	while (commentStartReg.test(value)) {
		commentAllIndexItems.push({
			type: 'start',
			index: commentStartReg.lastIndex - 4
		});
	}
	while (commentEndReg.test(value)) {
		commentAllIndexItems.push({
			type: 'end',
			index: commentEndReg.lastIndex - 3
		});
	}
	debug('comment item:%o', commentAllIndexItems);

	let isInComment = false;
	let isNotInited = false;
	const commentIndexs = [];
	commentAllIndexItems
		.sort(function(a, b) {
...
function _resplitLineStringsByCommentIndexs(lineStrings, commentIndexs) {
	...
	let tmpComment = [];
	let hasDealIndex = 0;
	let commentIndex = commentIndexs.shift();
	let isCommentEmpety = false;
	let isInComment = false;
	...
	function checkCommentIndexHandle(item, index) {
		...
		const endIndex = hasDealIndex + value.length;
		debug(
			'chek comment index start, hasDealIndex:%d, isInComment:%s, item value:%s index:%d',
			hasDealIndex,
			isInComment,
			value,
			index
		...
		if (commentIndex <= endIndex) {
			...
			if (isInComment) {
				// 3 为注释结束标志的长度
				const substrEndIndex = commentIndex - hasDealIndex + 3;
				let endPart = value.slice(0, substrEndIndex);
				// 从下一个值中补长度到可以被裁剪
				const endPartLackLen = substrEndIndex - endPart.length;
				debug('endPartLackLen:%d', endPartLackLen);
				endPart += _addItemLackValue(
					lineStrings,
					index,
					endPartLackLen
				);
				...
				if (!commentIndexs.length) isCommentEmpety = true;
				else commentIndex = commentIndexs.shift();
			}
			// 当前不是注释，进入注释队列收集模式
			// 也存在value长度不足的情况，但不需要从下一个item补
			else {
				const substrEndIndex = commentIndex - hasDealIndex;
				const startPart = value.slice(0, substrEndIndex);
				addResult(startPart);
			...
			if (isInComment) tmpComment.push(value);
	...
	if (!isCommentEmpety || tmpComment.length) {
		debug(
			'result err, isCommentEmpety:%s, tmpComment:%o, lineStrings:%o',
			isCommentEmpety,
			tmpComment,
			lineStrings
		);
...
function _addItemLackValue(lineStrings, index, lackLen) {
	...
	for (index++; lackLen && index < lineStrings.length; index++) {
		const nextItem = lineStrings[index];
		if (!nextItem) continue;
		const addstr = nextItem.value.slice(0, lackLen);
