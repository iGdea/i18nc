...
describe('#base', function() {
	...
	it('#getGenerator', function() {
		expect(jsoncode.getGenerator('1')).to.be(
			jsoncode.jsoncode.v1.generator
		);
		expect(jsoncode.getGenerator('a')).to.be(
			jsoncode.jsoncode.v1.generator
		);
		expect(jsoncode.getGenerator('A')).to.be(
			jsoncode.jsoncode.v1.generator
		);
		expect(jsoncode.getGenerator('H')).to.be(
			jsoncode.jsoncode.v2.generator
		);
	});
});
