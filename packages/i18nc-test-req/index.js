...
const debug = require('debug')('i18nc-test-req');
...
function requireAfterWriteReal(file, data) {
	...
	switch (type) {
		...
		case 'string':
			if (!/module\.exports *=/.test(data)) {
				const codeLens = data.match(/\n\S+/g);
				if (
					data.substr(0, 8) == 'function' &&
	...
	const realfile = path.join(exports.ROOT_PATH, file);
