...
program
	...
	.option(
		'   --i18n-handler-alias [name,name]',
		'I18N handler alias',
		cliUtil.argsToArray
	)
	.option(
		'   --ignore-scan-names [name,name]',
		'Ignore cacn handler names',
		cliUtil.argsToArray
	)
	...
	.option(
		'-f',
		['Force update total I18N Function', 'default: partial update'].join(
			COMMAND_INDENT
		) + '\n'
	)
	...
	.action(function(input, output, args) {
		const options = cliUtil.key2key(args, {
		...
		const obj = (options.codeModifyItems = {});
		...
		codeAction(cwd, input, output, options).catch(function(err) {
			console.log(err.stack);
		});
	});

program
	.command('check-wrap <input>')
	...
	.action(function(input, args) {
		...
		checkWrapAction(cwd, input, options).catch(function(err) {
			console.log(err.stack);
		});
	});
...
program
	.command('refs <string>')
	.description('Parse refs in po files. <e.g.> "1,1,0,7,subkey,*"')
	.option('-C, --no-color', 'Disable colored output.')
