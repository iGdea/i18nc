...
_.extend(ASTFunctionScope.prototype,
{
	...
	codeAndInfo: function(tmpCode, orignalCode, options)
	{
		...
		var dealAst		= [];
		var newCode		= [];
		var result		= scope.codeTranslateWordInfoOfSelf();
		...
		// 插入一个默认的翻译函数
		var i18nPlaceholderNew = new I18NPlaceholder(result.codeTranslateWords, orignalCode, options);
		var startPos = 0;
		...
		{
			ArrayPush.apply(result.dirtyAsts, scopeData.dirtyAsts);
			...
			ArrayPush.apply(result.codeTranslateWords.DEFAULTS, scopeData.codeTranslateWords.DEFAULTS);
			// 保留词频信息
			// codeTranslateWords.DEFAULTS = _.uniq(codeTranslateWords.DEFAULTS);
			ArrayPush.apply(result.codeTranslateWords.SUBTYPE_LINES, scopeData.codeTranslateWords.SUBTYPE_LINES);
			// 保留词频信息
			...
			{
				var arr = result.codeTranslateWords.SUBTYPES[subtype]
					|| (result.codeTranslateWords.SUBTYPES[subtype] = []);
			...
			ArrayPush.apply(result.i18nArgsTranslateWords.DEFAULTS, scopeData.i18nArgsTranslateWords.DEFAULTS);
			result.i18nArgsTranslateWords.DEFAULTS = _.uniq(result.i18nArgsTranslateWords.DEFAULTS);
			_.each(scopeData.i18nArgsTranslateWords.SUBTYPES, function(list, subtype)
			{
				var arr = result.i18nArgsTranslateWords.SUBTYPES[subtype]
					|| (result.i18nArgsTranslateWords.SUBTYPES[subtype] = []);

				ArrayPush.apply(arr, list);
				result.i18nArgsTranslateWords.SUBTYPES[subtype] = _.uniq(result.i18nArgsTranslateWords.SUBTYPES[subtype]);
			});
		});
		...
		result.funcTranslateWords = funcTranslateWords;
		// 处理后，正在使用的翻译数据 (包含子作用域)
		result.usedTranslateWords = usedTranslateWords;
