...
module.exports = function(code, options)
{
	...
	return scopeHanlder(scope, code, code, 0, options);
}

function scopeHanlder(scope, code, orignalCode, fixIndex, options)
{
	var newCode		= [];
	var dirtyWords	= [];
	var dealAst		= [];
	...
	var startPos = scope.ast.range[0]-fixIndex;
	if (scope.ast.type == 'BlockStatement') startPos++;
	...
	dealAst.sort(function(a, b)
		...
		.forEach(function(item)
		{
			...
			switch(item.type)
			{
				...
				case 'scope':
					var scopeData = scopeHanlder(item.scope, tmpCode.slice(startPos, endPos), orignalCode, startPos, options);
	...
	var isNeedSetNewI18NHanlder = !i18nPlaceholders.length
		&& scope.translateWordAsts.length
		// 并且，有需要处理的数据
		// 如果没有需要翻译的数据，那么也不需要添加函数
		&& (codeTranslateWords.DEFAULTS.length || Object.keys(codeTranslateWords.SUBTYPES).length);

	if (!isNeedSetNewI18NHanlder)
	{
	...
	codeTranslateWords.DEFAULTS = _.uniq(codeTranslateWords.DEFAULTS).sort();
	var tmpSUBTYPES = {};
	_.each(codeTranslateWords.SUBTYPES, function(list, subtype)
	{
		tmpSUBTYPES[subtype] = _.uniq(list).sort();
	});
	codeTranslateWords.SUBTYPES = tmpSUBTYPES;
