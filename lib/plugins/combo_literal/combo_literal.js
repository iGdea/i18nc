...
var debug		= require('debug')('i18nc-core:combo_literal');
...
exports.combo = function combo(ast, options)
{
	...
	var newAst = astUtils.asts2plusExpression(asts, AST_FLAGS.FROM_SPLICED_LITERAL);
	newAst.range = ast.range;
	astUtils.mergeAstFlag(newAst, ast);
...
exports.revert = function revert(lineStrings, comboAsts, options)
{
	...
	comboAsts.forEach(function(ast)
	{
		var astValue = ast.type == 'CallExpression'
				? astUtils.getI18NLiteral(ast, options.I18NHandlerName)
				: ast.value;
		...
		return this.asts.map(function(item)
			{
				return item.type == 'CallExpression'
					? astUtils.getI18NLiteral(item, options.I18NHandlerName)
					: item.value;
			})
			.join('');
	},
...
function _appendBinaryExpression(ast, options)
{
	...
	switch(ast.type)
	{
		...
		case 'CallExpression':
			var arg0Value = astUtils.getI18NLiteral(ast, options.I18NHandlerName);
