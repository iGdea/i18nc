...
var fs			= require('fs');
var debug		= require('debug')('i18nc:i18n_function_utils');
		...
		ast.declarations.forEach(function(var_ast)
		{
			...
			switch(name)
			{
				...
				case '__TRANSLATE_JSON__':
					...
					{
						var key = _getConstValueFromAst(translate_json_ast.key);
						...
						switch(key)
						{
							case 'DEFAULTS':
								...
								{
									var lan = _getConstValueFromAst(lan_ast.key);
							...
							case 'SUBTYPES':
								...
								translate_json_ast.value.properties.forEach(function(lan_ast)
								{
									...
									{
										var subkey = _getConstValueFromAst(subtype_ast.key);
...
function _dealTranslateDataJSON(ast)
{
	...
	{
		var name = _getConstValueFromAst(proto_ast.key);
		...
		{
			var arr = _logicalJsonAst2arr(val_ast);
			...
			for(var i = arr.length; i--;)
			{
				switch(index)
				{
					...
					default:
						debug('ignore translate word, key:%s val:%s, index:%d', name, arr[i], i);
				}
...
function _getConstValueFromAst(ast)
{
