		...
		ast.declarations.forEach(function(var_ast)
		{
			...
			switch(name)
			{
				...
				case '__TRANSLATE_JSON__':
					...
					// 通过options的函数处理一下
					if (initVal.type != 'ObjectExpression'
						&& options
						&& options.loadTranslateJSONByAst)
					{
						initVal = options.loadTranslateJSONByAst(initVal, options);
					}
...
function _translateSubtreeAst2json(ast)
{
	...
	ast.properties.forEach(function(proto_ast)
	{
		...
		{
			_logicalJSONAst2arr(val_ast, info);
		}
	});
 ...
 * a || b || c  =>  [c, b, a]
 */
function _logicalJSONAst2arr(ast, result)
{
...
function _appendLogicalExpression(ast, result)
{
	...
	{
		_logicalJSONAst2arr(ast, result);
	}
}
