...
{
	var ret = _.extend({}, exports.defaults);

	_.each(obj, function(val, key)
	{
		if (val || optionFalseKeys[key])
			ret[key] = val;
		else
			debug('option val == false, key:%s, val:%o', key, val);
	});

	// 直接设置为false
	// 避免生成无用的key
	if (ret.isMinI18Nhanlder) ret.isInjectAllTranslateWords = false;
