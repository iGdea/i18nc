...
var comboLiteralAst	= require('./ast_combo_literal').comboLiteralAst;
...
_.extend(ASTCollector.prototype,
{
	...
	_dealLiteral: function(ast)
	{
		...
		if (cutWordReg)
		{
			...
			if (splitTranslateWordArr.length > 1)
			{
				...
				dealWordsArr.push({translateWord: false, value: splitTranslateWordArr.pop()});
			}
		}
		...
		dealWordsArr = emitData.words;

		if (dealWordsArr && dealWordsArr.length)
		{
			var translateWords = [];
			...
			if (translateWords.join('') != value)
			{
				debug('emitData.words value is not eql original value:%o => %s', translateWords, value);
				comboAsts = null;
			}

			var retArr = comboAsts ? self._revertComboAst(dealWordsArr, comboAsts) : [{ast: ast, translateWords: translateWords}];

			return retArr.map(function()
				{
					var newAst = astUtils.asts2plusExpression(self._txts2asts(dealWordsArr));
	...
	// n:1
	// n:n （有一个是交叉的）
	_revertComboAst: function(dealWordsArr, comboAsts)
	{
		var result = [];
		...
		dealWordsArr.forEach(function(item)
		{
			...
			{
				translateWordStartMap[strlen] = item;
			}
		...
		var translateWordItemEnd = 0;
		var currentTranslateWordContainAsts = [];
		var currentAstContainTranslateWord = [];
		comboAsts.forEach(function(ast)
		{
			var astValue = ast.type == 'CallExpression' ? astUtils.getI18NLiteral(ast, options.handlerName) : ast.value;
			...
			do
			{
				if (!translateWordItem)
				{
					...
					if (translateWordItem)
					{
						translateWordItemEnd = strOffset+translateWordItem.value.length;
					}
				}
				else if (strOffset > translateWordItemEnd)
				{
				...
				if (translateWordItem)
				{
					if (!isJoin)
					{
						...
						currentAstContainTranslateWord.push(translateWordItem);
					}
				}
	...
	_comboLiteral: function(ast)
	{
		...
		return comboLiteralAst(ast, self.options);
	},
