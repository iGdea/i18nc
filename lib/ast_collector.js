	...
	scan: function(scope, ast)
	{
		...
		switch(ast.type)
		{
			...
			case 'CallExpression':
				...
				// 调用I18N
				else if (calleeName == self.options.I18NHandlerAlias)
				{
		...
		if (blockModifier == BLOCK_MODIFIER.SKIP_SACN
			|| blockModifier == BLOCK_MODIFIER.SKIP_SACN+'@'+self.options.I18NHandlerAlias)
		{
		...
		_.each(ast, function(item, ast_key)
		{
			...
			if (result2 && result2.length)
			{
				...
				if (blockModifier == BLOCK_MODIFIER.SKIP_REPLACE
					|| blockModifier == BLOCK_MODIFIER.SKIP_REPLACE+'@'+self.options.I18NHandlerAlias)
				{
