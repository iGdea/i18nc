...
var astComboLiteral		= require('./ast_combo_literal');
...
_.extend(ASTCollector.prototype,
{
	...
	_dealLiteral: function(ast)
	{
		...
		if (!dealWordsArr || !dealWordsArr.length) return;
		...
		if (comboAsts)
		{
			...
			retArr = astComboLiteral.revert(dealWordsArr, comboAsts, self.options);
		}

		if (!retArr)
		{
			retArr = [{ast: ast, translateWords: translateWords}];
		}
