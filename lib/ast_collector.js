...
var BLOCK_MODIFIER2		= _.invert(BLOCK_MODIFIER);
...
{
	collect: function(ast, scopeType, defaultSubtype)
	{
		var scope = new ASTFunctionScope(ast, scopeType);
		scope.defaultSubtype = defaultSubtype;
	...
	{
		return astTpl.CallExpression(this.options.I18NHandlerAlias, [astTpl.Literal(txt)]);
	},
	...
	scan: function(scope, ast)
	{
		...
		switch(ast.type)
		{
			case 'FunctionDeclaration':
				...
				if (handlerName == self.options.I18NHandlerAlias)
				{
				...
				{
					scope.subScopes.push(self.collect(ast.body, null, scope.defaultSubtype));
				}
			...
			case 'CallExpression':
				...
				switch(calleeName)
				{
					...
					case 'define':
						...
						{
							scope.subScopes.push(self.collect(defineLastArg.body, 'define factory', scope.defaultSubtype));
							return;
						}
						break;

					case self.options.I18NHandlerAlias:
						...
						if (i18nArgReulst.subtypeAst === undefined)
						{
							i18nArgReulst.subtypeAst = scope.defaultSubtype;
						}
		...
		if (blockModifier)
		{
			switch(blockModifier.type)
			{
				case BLOCK_MODIFIER.SKIP_SACN:
					if (!blockModifier.data || blockModifier.data == self.options.I18NHandlerAlias)
					{
				...
				case BLOCK_MODIFIER.DEFAULT_SUBTYPE:
					scope.subScopes.push(self.collect(ast, null, blockModifier.data));
					return;
			}
		}
		...
		_.each(ast, function(item, ast_key)
		{
			...
			if (result2 && result2.length)
			{
				...
				// 不替换成函数
				if (blockModifier
					&& blockModifier.type == BLOCK_MODIFIER.SKIP_REPLACE
					&& (!blockModifier.data || blockModifier.data == self.options.I18NHandlerAlias))
				{
	...
	_getBlockModifier: function(ast)
	{
		...
		if (!val) return;
		var info = val.split('@');
		if (!BLOCK_MODIFIER2[info[0]]) return;

		astUtils.setAstFlag(astBodyFirst, AST_FLAGS.BLOCK_MODIFIER);
