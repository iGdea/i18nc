...
var spliceLiteralAst	= require('./ast_splice_literal').spliceLiteralAst;
...
_.extend(ASTCollector.prototype,
{
	...
	scan: function(scope, ast, parent, itemName)
	{
		...
		switch(ast.type)
		{
			...
			case 'BinaryExpression':
				var splicedLiteralAst = self.spliceLiteral(ast);
				if (splicedLiteralAst)
				{
					return self.scan(scope, splicedLiteralAst, parent, itemName);
				}
				break;
		}
	...
	dealLiteral: function(ast, parent, itemName)
	{
		...
		// 同时包含非html标签的其他字符
		var dealArr = [];
		var cutWordReg = this.options.cutWordReg;
		...
		if (cutWordReg)
		{
			...
			{
				var translateWords = value.match(cutWordReg);
		...
		dealArr = emitData.words;

		if (dealArr && dealArr.length)
		{
			var result = astUtils.asts2plusExpression(this.txtarr(dealArr));
			if (result)
			{
				var translateWords = [];
				dealArr.forEach(function(item)
				{
	...
	spliceLiteral: function(ast)
	{
		var self = this;
		if (self.options.spliceLiteralMode == 'NONE'
			|| astUtils.checkAstFlag(ast, AST_FLAGS.SELF_CREATED))
		{
			return;
		}

		return spliceLiteralAst(ast, self.options);
	},
