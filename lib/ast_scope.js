	...
	squeeze: function(isKeepDefineFactoryScope, options)
	{
		...
		self.subScopes.forEach(function(scope)
		{
			...
			if (scope2.I18NHanlderAsts.length
				...
				|| (self.type == 'top' && options.isClosureWhenInsertedHead))
			{
			...
			.forEach(function(item)
			{
				...
				switch(item.type)
				{
					case 'I18NAliasCallee':
						...
						myCode = escodegen.generate(myAst, optionsUtils.escodegenOptions);
						myCode = wordsUtils.unescape4escodegen(myCode);
						newCode.push(myCode);
						break;
					...
					case 'translateWord':
						...
						{
							myCode = escodegen.generate(ast.__i18n_replace_info__.newAst, optionsUtils.escodegenOptions);
							myCode = wordsUtils.unescape4escodegen(myCode);
							newCode.push(myCode);
						}
						break;
