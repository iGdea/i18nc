...
_.extend(ASTScope.prototype,
{
	...
	codeAndInfo: function(tmpCode, orignalCode, options)
	{
		...
		// 插入一个默认的翻译函数
		var I18NPlaceholderNew = new I18NPlaceholder(codeTranslateWordsJSON, orignalCode, options);
		var startPos = 0;
		...
		if (I18NPlaceholderNew.getRenderType() == 'complete'
			...
			&& scope.type == 'top')
		{
			debug('closure code, scope type:%s', scope.type);
			resultCode = ';(function(){\n'+resultCode+'\n})();'
		}
		...
		var selfScopeData =
		{
			...
			words           : new TranslateWords(selfTWords.codeTranslateWords, selfFuncTranslateWords, currentI18NHanlder.getTranslateJSON()),
		};
...
_.extend(CodeTranslateWords.prototype,
{
	...
	{
		return this.asts.filter(function(item)
			{
...
_.extend(TranslateWords.prototype,
{
	toJSON: function()
	{
		return {
			...
			funcTranslateWords: this.funcTranslateWords,
			usedTranslateWords: this.usedTranslateWords,
		};
	}
});
