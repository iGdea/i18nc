...
_.extend(ASTScope.prototype,
{
	...
	codeAndInfo: function(tmpCode, originalCode, options)
	{
		...
		dealAst.sort(function(a, b)
			...
			.forEach(function(item)
			{
				...
				switch(item.type)
				{
					...
					case 'scope':
						var scopeData = item.scope.codeAndInfo(tmpCode.slice(startPos, endPos), originalCode, options);
		...
		var usedTranslateWords = new UsedTranslateWords(currentI18NHanlder.getTranslateJSON());
...
_.extend(CodeInfoResult.prototype,
{
	...
	allDirtyAsts: function()
	{
		var self = this;
		var result = self.dirtyAsts.slice();

		self.subScopeDatas.forEach(function(item)
		{
			ArrayPush.apply(result, item.allDirtyAsts());
		});
