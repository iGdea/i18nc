...
_.extend(LiteralHandler.prototype,
{
	...
	_regExpHandler: function(ast)
	{
		...
		ast.__i18n_replace_info__ =
		{
			...
			translateWords	: self._getTranslateWordsFromLineStrings(lineStrings)
		};
		...
		return retArr.map(function(item)
			{
				...
				{
					newAst			: newAst,
					translateWords	: self._getTranslateWordsFromLineStrings(item.lineStrings)
				};
	...
	_getTranslateWordsFromLineStrings: function(lineStrings)
	{
		var translateWords = [];
	...
	split: function(value)
	{
		...
		// 对分词进行美化
		if (self.options.cutWordBeautify.RemoveTplComment)
			lineStrings = cutwordBeautify.removeTplComment(lineStrings, value);
		if (self.options.cutWordBeautify.SplitByEndSymbol)
			lineStrings = cutwordBeautify.splitByEndSymbol(lineStrings, self.options);
		if (self.options.cutWordBeautify.KeyTrim)
