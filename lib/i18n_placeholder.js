...
_.extend(I18NPlaceholder.prototype,
{
	...
	{
		var newTranslateJSON = this.getTranslateJSON();
		i18nFunctionGenerator.fillNoUsedCodeTranslateWords(newTranslateJSON, this.codeTranslateWords);
		var newTranslateJSONCode = i18nFunctionGenerator.genTranslateJSONCode(newTranslateJSON);

		var emitData =
		{
			options					: this.options,
			newTranslateJSON		: newTranslateJSON,
			newTranslateJSONCode	: newTranslateJSONCode,
		};

		emitter.emit('newTranslateJSON', emitData);

		return ''+emitData.newTranslateJSONCode;
	},
