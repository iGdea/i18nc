...
_.extend(I18NPlaceholder.prototype,
{
	toString: function()
	{
		...
		switch(renderType)
		{
			...
			default:
				...
				{
					newCode = '\n'
						+ this.options.I18NhandlerTpl_NewHeaderCode
						+ newCode
						+ this.options.I18NhandlerTpl_NewFooterCode
						+ '\n';
				}
	...
	getRenderType: function()
	{
		...
		if (!this.options.codeModifiedArea.I18NHandler) return 'original';
	...
	{
		var funcInfo = this.parse();
		var noFuncTranslateWords = this.options.isIgnoreI18NHandlerTranslateWords;
	...
	_updateTotalCode: function()
	{
		...
		var GetLanguageCode = this.options.I18NhandlerTpl_GetLanguageCode;

		if (typeof GetLanguageCode == 'function')
		{
			GetLanguageCode = GetLanguageCode.toString().trim()
				.replace(/^function\s*\(/, 'function a(');
			GetLanguageCode = i18nTpl.min(GetLanguageCode)
				.replace(/^function \s*\w+\s*\(/, 'function(');
			GetLanguageCode = '('+GetLanguageCode+')';
		}

		var LanguageVars = this.options.I18NhandlerTpl_LanguageVars || {};
		...
		{
			renderData.globalHandlerName = funcInfo.globalHandlerName;
			...
			newCode = i18nTpl.renderGlobal(renderData);
		}
		else
		{
			debug('i18n fucntion renderdata: %o', renderData);
			newCode = i18nTpl.render(renderData);
		}
