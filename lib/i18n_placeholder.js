...
_.extend(I18NPlaceholder.prototype,
{
	toString: function()
	{
		...
		console.log('renderType:%s', renderType);
	...
	_updatePartialCode: function()
	{
		...
		if (this.oldAst)
		{
			codeIndent = this.completedCode.slice(0, json_ast_range[0])
					.split('\n').pop();
	...
	_updateTotalCode: function()
	{
		...
		var renderData =
		{
			...
			useOnlyThisLanguage	: this.options.useOnlyThisLanguage,
			getGlobalCode		: this.options['I18NhandlerCode:getGlobalCode'],
		...
		if (this.oldAst)
		{
			codeIndent = this.completedCode.slice(0, this.oldAst.range[0]).split('\n').pop();
			codeIndent = codeIndent.match(/^\s*/)[0];
			if (!codeIndent) codeIndent = '';
