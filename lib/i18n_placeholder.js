...
_.extend(I18NPlaceholder.prototype,
{
	...
	_updatePartialCode: function()
	{
		...
		var firstPart = this.completedCode.slice(this.oldAst.range[0], json_ast_range[0]);
		var indent = /\n[\t ]*$/.test(firstPart) ? null :  '\t';
		var newJSONCode = this._getRenderTranslateJSONCode();

		// 压缩这个代码的时候，需要加上()
		// 不然esprima会报错
		newJSONCode = this._beautifyCode('('+newJSONCode+')', funcInfo.__TRANSLATE_JSON__ast, indent).slice(1);
	...
	_updateTotalCode: function()
	{
		...
		var renderData =
		{
			handlerName			: this.options.I18NHandlerAlias,
			FILE_KEY			: funcInfo.__FILE_KEY__,
			FUNCTION_VERSION	: optionsUtils.I18NFunctionVersion,
			pickFileLanguages	: this.options.pickFileLanguages.join(','),
		...
		newCode = this._beautifyCode(newCode, this.oldAst);
	...
	_beautifyCode: function(code, ast, indent)
	{
		if (this.options.isMinI18Nhanlder)
		{
