...
var tplContentSimple	= fs.readFileSync(__dirname+'/i18n_function_tpl/simple.tpl', {encoding: 'utf8'});


function tpl2render(content, data)
{
	return content.replace(/\$(\w+)/g, function(all, key)
	{
		return data[key] || '';
	});
}
...
{
	return tpl2render(tplContent, data).trim();
}

exports.renderSimple = renderSimple;
function renderSimple(data)
{
	return tpl2render(tplContentSimple, data).trim();
}
