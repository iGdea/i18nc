...
function _getOneTypeListData(maintype, word, mainData, FILE_KEY, subtype)
{
	...
	lans.forEach(function(lan)
	{
		var tmp = dbFileKeyTranslates[lan] || funcTranslates[lan];
		if (!tmp || !tmp.length)
		{
...
function _wordmap2ast(words)
{
	...
	Object.keys(words).sort()
		.forEach(function(val)
		{
			...
			debug('wordmap2ast val:%s, translateWord:%o', val, translateWord);
			...
			var retAst = astTpl.Property(val, astUtils.constVal2ast(translateWord));
			result.push(retAst);
