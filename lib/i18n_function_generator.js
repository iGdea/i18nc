...
var fs				= require('fs');
var debug			= require('debug')('i18nc-core:i18n_function_generator');
var escodegen		= require('escodegen');
var template		= require('art-template');
var astUtils		= require('./ast_utils');
var astTpl			= require('./ast_tpl');
var optionsUtils	= require('./options');
var chooseWord		= require('./words_db_or_code').choose;

var tplContent		= fs.readFileSync(__dirname+'/i18n_function_tpl/full.tpl', {encoding: 'utf8'});
var functionTpl		= template.compile(tplContent);

tplContent			= fs.readFileSync(__dirname+'/i18n_function_tpl/simple.tpl', {encoding: 'utf8'});
var functionTpl2	= template.compile(tplContent);
...
function _getOneTypeListData(maintype, word, mainData, FILE_KEY, subtype)
{
	...
	var lans = {};
	var setFileLanguages = mainData.setFileLanguages
			&& mainData.setFileLanguages.length
			&& mainData.setFileLanguages;

	_.each(mainData.dbTranslateWords, function(lanInfo, lan)
	{
		if (!lanInfo) return;
		if (setFileLanguages && setFileLanguages.indexOf(lan) == -1) return;
		...
		if (subLanInfo && word in subLanInfo)
		{
			...
			dbNormalTranslates[lan] = [ undefined, subLanInfo[word] ];
		}


		// 通过FILE_KEY来查找的数据结构都是ok的
		// 不需要像上面，进行结构体转化
	...
	lans.forEach(function(lan)
	{
		...
		if (!tmp || !tmp.length)
		{
			tmp = dbNormalTranslates[lan];
			debug('switch dbNormalTranslates:%s result:%o', lan, tmp);
		}
...
function _wordmap2ast(words)
{
	...
	Object.keys(words).sort()
		.forEach(function(val)
		{
			...
			if (!keyAstVals.length)
			{
				emptyTranslateComments.push(astTpl.LineComment(' '+JSON.stringify(val)+':'));
				return;
			}
			...
			// 转化成 “||” 之后，保存到result中
			var retAst = astTpl.Property(val, astUtils.asts2orExpression(keyAstVals));
			result.push(retAst);
	...
	if (emptyTranslateComments.length)
	{
		...
		result[result.length-1].leadingComments = emptyTranslateComments;
	}
