...
function _getOneTypeListData(maintype, word, mainData, FILE_KEY, subtype)
{
	...
	if (!lans.length && setFileLanguages)
	{
		lans = [ setFileLanguages[0] ];
	}
	...
	lans.forEach(function(lan)
	{
		...
		if (!tmp || !tmp.length)
		{
			...
			console.log('switch dbNormalTranslates:%s result:%o', lan, tmp);
		}

		// 如果没有tmp，就弄一个空数组后面会做注释处理
		result[lan] = tmp || [];
	});
...
function _wordmap2ast(words)
{
	...
	var retAst = astTpl.ObjectExpression(result);
	if (emptyTranslateComments.length)
	{
		if (result.length)
		{
			// result[result.length-1].trailingComments = emptyTranslateComments;
