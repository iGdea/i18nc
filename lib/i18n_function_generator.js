...
function _getOneTypeListData(maintype, word, mainData, FILE_KEY, subtype)
{
	...
	{
		var tmp = _chooseWord(funcTranslates[lan], dbFileKeyTranslates[lan])
 ...
 */
function _chooseWord(funcTranslate, dbFileKeyTranslate)
{

	if (!funcTranslate) funcTranslate = [];
	if (!dbFileKeyTranslate) dbFileKeyTranslate = [];

	var dbFileKeyTranslateLast = dbFileKeyTranslate[dbFileKeyTranslate.length-1];
		...
		switch(funcTranslate.length)
		{
			...
			case 2:
				...
				switch(dbFileKeyTranslate.length)
				{
					...
					default:
						var lanReuslt = funcTranslate.slice();
						lanReuslt.push(dbFileKeyTranslateLast);
						return lanReuslt;
						break;
				}
			...
			default:
				...
				{
					lanReuslt[1] = funcTranslate[funcTranslate.length-1];
	...
	else if (funcTranslate.length > 1)
	{
	...
	else if (dbFileKeyTranslate.length)
	{
		// 如果只有db中有
		// 那么输出完整的db数据
		// 不要只输出最后值
